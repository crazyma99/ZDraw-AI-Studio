...
class LoRAMerger:
    ...
    @staticmethod
    def scan_loras(directory: str):
        """扫描指定目录下的 .safetensors 文件"""
    ...
    def apply_lora_weights(self, lora_path: str, delta_scale: float):
        """应用 LoRA 权重增量 (支持正负/小数)"""
        if not os.path.exists(lora_path):
            raise FileNotFoundError(f"LoRA file not found: {lora_path}")

        print(f"🎨 [LoRA Manager] 应用 LoRA 变更: {os.path.basename(lora_path)} (增量: {delta_scale:+.2f})...")
        
        tensors = safetensors.torch.load_file(lora_path)
        self._merge_lora_weights(tensors, delta_scale)
        self.loaded_path = lora_path

    def load_lora_weights(self, lora_path: str, lora_scale: float = 1.0):
        """兼容旧接口"""
        try:
            self.apply_lora_weights(lora_path, lora_scale)
